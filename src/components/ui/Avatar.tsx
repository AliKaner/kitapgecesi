...
/* User/author avatar — round, image or a centered "user" icon fallback.
   `ring` adds a soft sand ring used on the feed; `size` is the diameter in
   px (or xs/sm/md/lg/xl). */
  ...
  return (
    ...
    >
      {src ? (
        ...
        <Icon name="user" size={Math.max(12, Math.round(px * 0.55))} style={{ opacity: 0.5 } as CSSProperties} />
      )}
    </span>
  );
}
