...
/* KitapGecesi icon set — clean 24px line icons (Lucide-style geometry:
   2px-ish stroke, round caps/joins). Self-contained, no CDN dependency.
   `star` and a few glyphs support a filled variant via the `fill` prop. */
...
const PATHS: Record<string, ReactElement> = {
  ...
  list: (
    <>
      ...
      <line x1="3.5" x2="3.51" y1="6" y2="6" />
      <line x1="3.5" x2="3.51" y1="12" y2="12" />
      <line x1="3.5" x2="3.51" y1="18" y2="18" />
    </>
  ),
  ...
  "eye-off": (
    <>
      ...
      <path d="M6.61 6.61C4.13 8.36 2.5 11 1 12c0 0 4 7 11 7a9.26 9.26 0 0 0 5.39-1.61" />
...
export interface IconProps extends Omit<SVGProps<SVGSVGElement>, "fill" | "color"> {
  name: IconName;
  size?: number;
  strokeWidth?: number;
  fill?: boolean;
...
export function Icon({ name, size = 20, strokeWidth = 1.75, fill = false, color, style, className, ...rest }: IconProps) {
  ...
  return (
    <svg
      ...
      className={className}
      style={{ display: "block", flex: "none", color, ...style } as CSSProperties}
      aria-hidden="true"
      {...rest}
    >
      {glyph || null}
